...
class ExpandField extends BaseModel
{
    ...
    {
        $info = self::find($id);
        $expand = Expand::find($info->expand_id);
        Schema::table('expand_data_' . $expand->table, function (Blueprint $table) use ($data, $info) {
    ...
    public function deleteById(int $id)
    {
        ...
        $expand = Expand::find($info->expand_id);
        Schema::table('expand_data_' . $expand->table, function (Blueprint $table) use ($info) {
            $table->dropColumn($info->field);
        });
        return $info->delete();
    }

    public function getFieldHtml($expandData = null)
    {
        if (! isset($this->id)) {
            return '';
        }
...
EOF;
    ...
    public function getFieldValue($expandData)
    {
        if (! isset($this->id)) {
            return '';
        }
        $value = is_array($expandData) ? $expandData[$this->field] : $expandData->{$this->field};
        switch ($this->type) {
            // 单行文本框
            case 1:
                switch ($this->property) {
