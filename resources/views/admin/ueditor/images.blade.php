...
<link rel="stylesheet" type="text/css" href="/lib/webuploader/upfile/upfilestyle.css">
      ...
      <tr>
        <td>
          ...
          <select id="thumbnail_cutout" name="thumbnail_cutout">
            <option @if(config('system.thumbnail_cutout') == 1)selected @endif value="1">裁剪</option>
            <option @if(config('system.thumbnail_cutout') == 0)selected @endif value="0">按比例</option>
          </select>
        </td>
      </tr>
	  <tr>
...
(function( $ ){
    ...
    $(function() {
        ...
        if ( !WebUploader.Uploader.support('flash') && WebUploader.browser.ie ) {
            ...
            if (flashVersion) {
                (function(container) {
                    ...
                    var swf = '/lib/webuploader/upfile/expressInstall.swf';
                    // insert flash object
        ...
        uploader = WebUploader.create({
            ...
            chunked: false,
            chunkSize: 512 * 1024,
            server: '/admin/ueditor/index?origin={{ $origin }}&action=uploadimage',
            // runtimeOrder: 'flash',
        ...
        uploader.on('uploadSuccess', function (file, json) {
            ...
            var thumbnail_url = typeof(json.thumbnail_url) == "undefined" ? json.url : json.thumbnail_url;
            try {
                if (json.state == 'SUCCESS') {
                    ...
                    var html="<li>\
							  <div class='pic' id='images_button'>\
								  <img src='" + thumbnail_url + "' width='125' height='105' />\
								  <input  id='{{ $id }}_url[]' name='{{ $id }}_url[]' type='hidden' value='" + json.url + "' />\
								  <input  id='{{ $id }}_thumbnail_url[]' name='{{ $id }}_thumbnail_url[]' type='hidden' value='" + thumbnail_url + "' />\
							  </div>\
					...
					$('#{{ $id }}_list',parent.document).append(html);
                } else {
